...
pub fn load_lora(path: &Path, device: &candle_core::Device) -> Result<Vec<LoraTensorPair>> {
    let tensors = candle_core::safetensors::load(path, device)
        .map_err(|e| DiffusionError::Model(format!("LoRA {}: {e}", path.display())))?;
    ...
    for (name, tensor) in tensors {
        ...
        if let Some(target) = name.strip_suffix(".lora_down.weight").or_else(|| name.strip_suffix(".lora_A.weight")) {
            ...
            let value = tensor
                ...
                .map_err(|e| DiffusionError::Model(e.to_string()))?
                .first()
                .copied()
                .unwrap_or(1.0);
    ...
    if result.is_empty() {
        return Err(DiffusionError::Model("LoRA contains no recognized tensor pairs".into()));
    }
    Ok(result)
}
...
pub fn load_conditioning(path: &Path, device: &candle_core::Device) -> Result<BTreeMap<String, Tensor>> {
    let tensors = candle_core::safetensors::load(path, device)
        .map_err(|e| DiffusionError::Model(format!("conditioning {}: {e}", path.display())))?;
